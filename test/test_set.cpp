...
TEST_CASE("Test redis set")
{
    auto f = asio::co_spawn(
        ...
        [&] -> asio::awaitable<void>
        {
            ...
            {
                auto ret = co_await async_redis->async_sadd(key, data, asio::use_awaitable);
...
TEST_CASE("Test redis sort set")
{
    auto f = asio::co_spawn(
        ...
        [&] -> asio::awaitable<void>
        {
            ...
            {
                co_await async_redis->async_del(sorted_set_key, asio::use_awaitable);
                auto ret = co_await async_redis->async_zadd(sorted_set_key, "a", 99, sw::redis::UpdateType::ALWAYS,
                                                            true, asio::use_awaitable);
                REQUIRE(ret.has_value());
                REQUIRE(ret.value() == 1);
            }
            {
            ...
            }
            {
                ...
                auto ret = co_await async_redis->async_zrangebyscore<std::vector<std::string>>(
                    sorted_set_key, 0, 5, false, offset, count, asio::use_awaitable);
                REQUIRE(ret.has_value());
                REQUIRE(ret.value().size() == 2);
            }
            co_return;
