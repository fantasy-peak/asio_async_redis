    ...
    auto f = asio::co_spawn(
        ...
        [&] -> asio::awaitable<void> {
            ...
            }
            {
                ...
                auto data = co_await async_redis->async_xrange(
                    stream_name, "-", "+", std::make_optional(1), asio::use_awaitable);
                REQUIRE(data.value().size() == 1);
            }
            {
                auto ret = co_await async_redis->async_xread(
                    stream_name, id, std::chrono::milliseconds(5000), 100, asio::use_awaitable);
